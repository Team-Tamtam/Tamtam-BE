...
public class AIPromptService {
    ...
    public String buildMonthlyBudgetMessage(
        ...
        String GPT_PROMPT = "You are a financial assistant AI responsible for creating a personalized monthly budget. Your task is to analyze the provided data and generate a budget plan for the next month in JSON format. Ensure the budget is tailored for 30 days (one month). Assign categories reasonably, especially for essential costs like food and living expenses, but within the fixed total budget. Explain the reasoning behind your allocations in detail in Korean in the `reason` field. You should assign a REASONABLE budget for 'Next Month's Plans' and each category. \n\n" +
                ...
                "6. **Total Budget**: The maximum budget available for allocation next month (e.g., {0} KRW) There is no way you can exceed or underallocate the budget in there. THIS IS FIXED TOTAL BUDGET. At last you should sum up all of categories budget and if it doesn't fit to total fixed budget, you should arrange by applying proportions for all the categories so you should end up fit to total budget. THERE IS NO NEGOTIATION FOR THIS.\n\n" +
                "### Key Constraints:\n- After assigning budgets to each category, you **must** calculate the sum of all category budgets and compare it to the `total_budget`. If the sum does not match the `total_budget`, adjust the allocations proportionally to ensure that the sum equals the fixed total budget exactly. There is no negotiation for this.\n\n"  +
                "### Output Requirements:\nGenerate the budget in the following JSON format:\n{\n  \"monthly_budget\": {\n    \"total_budget\": {0},\n    \"category_budgets\": {\n      \"경조/선물\": 0,\n      \"교육/학습\": 0,\n      \"교통\": 0,\n      \"금융\": 0,\n      \"문화/여가\": 0,\n      \"반려동물\": 0,\n      \"뷰티/미용\": 0,\n      \"생활\": 0,\n      \"술/유흥\": 0,\n      \"식비\": 0,\n      \"여행/숙박\": 0,\n      \"온라인 쇼핑\": 0,\n      \"의료/건강\": 0,\n      \"자녀/육아\": 0,\n      \"자동차\": 0,\n      \"주거/통신\": 0,\n      \"카페/간식\": 0,\n      \"패션/쇼핑\": 0\n    }\n  },\n  \"reason\": \"Explain the reasoning behind the budget allocations in Korean. Include an explicit explanation of how the category budgets were adjusted to ensure that the total matches the fixed budget, if any adjustments were made. Example of response is like the following, \"이번 달 예산은 사용자님의 생활 패턴과 피드백을 꼼꼼히 반영해서 짜 보았어요.\n먼저, 필수적으로 지출해야 하는 고정비(주거/통신 150,000원, 교통 70,000원)는 꼭 필요한 만큼 먼저 배정했어요. 다음 달에는 기타 학원 등록을 원하셨기 때문에 교육/학습 항목에 200,000원을 충분히 배정했어요. 지난달 식비가 조금 초과되었다고 하셔서, 충분히 여유롭게 드실 수 있도록 살짝 늘렸어요. 카페/간식은 딱 적당히 사용하신 것 같아 이번에도 그대로 유지했어요.\n문화/여가 쪽은 사용이 적어서 20,000원으로 줄이고, 그 금액은 다른 꼭 필요한 항목에 넣었답니다. 또한, 다음 달 주말 여행을 계획하셨다고 하셔서, 여행/숙박 항목에 금액을 새로 배정했어요. 요청하신 다른 항목들 중에서 예산에 부담이 되는 부분은 조금 줄였지만, 최대한 반영하려 노력했답니다.\n술/유흥은 특별히 많이 필요하지 않으실 것 같아 조금 줄였고, 생활비는 꼭 필요한 만큼 그대로 유지했어요. 의료/건강 항목은 정기 검진 등 필요한 비용을 고려해서 10,000원을 추가했어요. 지난 달 피드백에 따라 쇼핑에서 지출을 줄여서 다음 달 주요 일정을 위한 돈을 모아보는 건 어떨까요? 다음 달도 편안하게 보내실 수 있기를 바라요. 😊\"\" \n}\n";
