...
public interface ExpenseRepository extends JpaRepository<Expense, Long> {
    @Query("SELECT COALESCE(SUM(e.amount), 0) FROM Expense e " +
            "WHERE e.user.userId = :userId AND e.date BETWEEN :startOfMonth AND :endOfMonth")
    BigDecimal findTotalExpenseAmountByUserAndMonth(Long userId, LocalDate startOfMonth, LocalDate endOfMonth);
